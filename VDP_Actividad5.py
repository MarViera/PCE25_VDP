...
def get_ips():# Función para ejecutar el script de powershell (IPs Activas)
    ...
    try:
        ...
        if not ips:
            raise ValueError("Sin IPs para trabajar")  #Manejo de error para lista de IP vacías
        print ("Obtención de IPs exitosa")
    except ValueError as ve:
        logging.error(f"Error: {ve}")  #Mensaje de error
        print(f"Error: {ve}")
        return []
    except subprocess.TimeoutExpired:      #Se agotó el tiempo de espera para recibir el script de powershell
        logging.error("Tiempo agotado para ejecución de script externo. Volver a intentar")
...
def checarIP(ip):
   ...
   try:
    ...
    except requests.exceptions.ConnectionError:     #Manejo de error de conexión con la api
        logging.error(f"No se pudo conectar a la API AbuseIPBD")
    return f"Error de conexión al consultar la IP {ip}"
